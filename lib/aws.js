...
export default function (__usable_environment) {
	...
	__usable_module.exports = ((a) => {
		function s3(opt) {
			...
			this.config = opt;
			this.AWS = awsSdkImport;
			this.on = a.on;
			if (
				(this.config.fakes3 =
					this.config.fakes3 || opt.fakes3 || __usable_process.env.fakes3)
			) {
				this.AWS.config.endpoint = this.config.endpoint =
					opt.fakes3 || this.config.fakes3 || __usable_process.env.fakes3;
				this.AWS.config.sslEnabled = this.config.sslEnabled = false;
				this.AWS.config.bucket = this.config.bucket =
					this.config.bucket.replace(".", "p");
			}
