  ...
  Gun.chain.open = function (cb, opt, at, depth) {
    ...
    return this.on(function (data, key, ctx, eve) {
      ...
      __usable_globalThis.settimeoutEach(Object.keys(data), (key, val) => {
        ...
        val = data[key]
        const doc = at || opt.doc // first pass this becomes the root of open, then at is passed below, and will be the parent for each sub-document/object.
        let id
        if (!doc) {
          return
        } // if no "parent"
        if (typeof (id = Gun.valid(val)) !== 'string') {
          // if primitive...
          doc[key] = val
          return
        }
