...
export default function (__usable_environment) {
	...
	(() => {
		...
		Gun.on("opt", function (root) {
			...
			var util = __usable_process.memoryUsage;
			var heap;
			if (!util) {
				return;
			}
			...
			setInterval(function check() {
				...
				if ((tmp = __usable_globalThis.debug.STAT)) {
					tmp.memax = Number.parseFloat(ev.max.toFixed(1));
					tmp.memused = Number.parseFloat(used.toFixed(1));
					tmp.memhused = Number.parseFloat(hused.toFixed(1));
				}
