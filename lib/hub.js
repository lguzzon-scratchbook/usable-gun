...
import chokidarImport from 'chokidar'
  ...
  __usable_module.exports = {
    watch: function watch (what, options) {
      options = options ?? {
        msg: true,
        hubignore: false,
        alias: nodeOsImport.userInfo().username
      }
      options.msg = options.msg ?? true
      options.hubignore = options.hubignore ?? false
      options.alias = options.alias ?? nodeOsImport.userInfo().username
      const modifiedPath = options.alias
      let watcher
      try {
            ...
            if (options.hubignore && path.includes('.hubignore')) {
              ...
              if (path[path.search(/^./gm)] === '/' || '.') {
                gun
                  ...
                  .get(
                    modifiedPath +
											path.split(nodeOsImport.userInfo().username)[1]
                  )
