...
export default function (__usable_environment) {
  ...
  const parse =
		...
		((t, cb, r) => {
		  ...
		  try {
		    cb(undefined, JSON.parse(t, r), json.sucks(Number(new Date()) - d))
		  } catch (e) {
		    cb(e)
		  }
		})
  var json =
		...
		((v, cb, r, s) => {
		  const d = Number(new Date())
		  try {
		    cb(undefined, JSON.stringify(v, r, s), json.sucks(Number(new Date()) - d))
		  } catch (e) {
		    cb(e)
		  }
		})
  ...
  function Mesh (root) {
    ...
    const puff = __usable_globalThis.settimeoutTurn || setTimeout
    const dup = root.dup
    ...
    var hear = (mesh.hear = function (raw, peer) {
      ...
      const S = (peer.SH = Number(new Date()))
      const tmp = raw[0]
      let msg
      ...
      if (tmp === '[') {
        parse(raw, (err, msg) => {
          ...
          (function go () {
            ...
            __usable_globalThis.debug.STAT &&
							__usable_globalThis.debug.STAT(S, Number(new Date()) - S, 'hear loop')
    ...
    (() => {
      ...
      mesh.hash = (msg, peer) => {
        ...
        json(
          ...
          function hash (err, text) {
            const ss = (s || (s = t = text || '')).slice(0, 32768) // 1024 * 32
      ...
      const say = (mesh.say = function (msg, peer) {
        let tmp
        ...
        DBG && (DBG.yh = Number(new Date()))
        if (!(raw = meta.raw)) {
          mesh.raw(msg, peer)
          return
        }
        DBG && (DBG.yr = Number(new Date()))
        if (!peer || !peer.id) {
          if (!__usable_globalThis.objectPlain(peer || opt.peers)) {
            return false
          }
          var S = Number(new Date())
          const P = opt.puff // TODO: .keys( is slow
          const ps = opt.peers
          let pl = Object.keys(peer || opt.peers || {})
          __usable_globalThis.debug.STAT &&
						__usable_globalThis.debug.STAT(S, Number(new Date()) - S, 'peer keys');
          (function go () {
            const S = Number(new Date())
            ...
            __usable_globalThis.debug.STAT &&
							__usable_globalThis.debug.STAT(S, Number(new Date()) - S, 'say loop')
            if (!pl.length) {
              return
            }
            puff(go, 0)
        ...
        __usable_globalThis.debug.STAT &&
					__usable_globalThis.debug.STAT(
					  S,
					  ((DBG || meta).yp = Number(new Date())) - (meta.y || S),
					  'say prep'
					)
        ...
        setTimeout(() => {
          __usable_globalThis.debug.STAT &&
						__usable_globalThis.debug.STAT(ST, Number(new Date()) - ST, '0ms TO')
          flush(peer)
        }, opt.gap) // TODO: queuing/batching might be bad for low-latency video game performance! Allow opt out?
        send(raw, peer)
        __usable_globalThis.debug.STAT &&
					ack === peer.SI &&
					__usable_globalThis.debug.STAT(S, Number(new Date()) - peer.SH, 'say ack')
      })
      ...
      mesh.raw = (msg, peer) => {
        ...
        if (!msg.dam && !msg['@']) {
          ...
          tmp = opt.peers
          for (const k in tmp) {
            const p = tmp[k] // TODO: Make it up peers instead!
        ...
        if ((put = meta.$put)) {
          ...
          json(tmp, (err, raw) => {
            ...
            __usable_globalThis.debug.STAT &&
							__usable_globalThis.debug.STAT(S, Number(new Date()) - S, 'say slice')
          })
          return
        }
        json(msg, res)
    ...
    mesh.hear.mob = (msg, peer) => {
      ...
      const one = peers[(__usable_globalThis.mathRandom() * peers.length) >> 0]
      if (!one) {
        return
      }
      mesh.bye(peer)
