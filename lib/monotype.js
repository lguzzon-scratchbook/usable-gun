...
export default function (__usable_environment) {
  ...
  var monotype =
		...
		((monotype) => {
		  monotype.range = function (n) {
		    ...
		    let s
		    var n = n || 0
		    const win = monotype.win || __usable_window
		  ...
		  monotype.restore = (R) => {
		    ...
		    if (win.getSelection) {
		      ...
		      if (doc.body.createTextRange) {
		        // <IE9
		        const ier = doc.body.createTextRange()
		  ...
		  monotype.reach = (i, c, o) => {
		    ...
		    let n = monotype.deep(o.$[0], -1).container
		    let t
		    while (n) {
		  ...
		  r.range = () => {
		    // console.log('----');
		    r.H = r.H || {}
		    let s = m.reach(r.s, r.root[0])
		    const st = s.$.text()
		    let e = m.reach(r.e, r.root[0])
		    const et = e.$.text()
		    const R = m.range()
  ...
  monotype.late = (r, opt) => {
    ...
    const strhml = (t) => t[0] === '<' && __usable_globalThis.$(t).length
    const jqtxt = (n) => n.jquery
      ? n
      : strhml(n)
