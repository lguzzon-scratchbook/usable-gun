...
export default function (__usable_environment) {
	...
	(() => {
		...
		var edide = {};
		edide.global = (() =>
			typeof __usable_globalThis !== "undefined"
				? __usable_globalThis
		...
		edide.strRandom = (() =>
			(limit = 20) =>
				(__usable_globalThis.mathRandom() + "").slice(
					2,
					+(limit + 1) + 1 || 9e9,
				)).call((MODSELF = {}), edide, MODSELF);
		...
		edide.reactiveWithFilters = ((edide) =>
			(initialVars = {}, filters = {}) => {
				var handler;
				var id;
				var key;
				...
				unvar = new Proxy(todoMap, {
					...
					set: (map, prop, value) =>
						edide.var.values.set(`${id}.${prop}`, value),
				});
		...
		edide.strParsesToNumber = function (edide, strParsesToNumber) {
			...
			var strParsesToNumber = (str) => !Number.isNaN(Number.parseInt(str));
				...
				switch (track[ts.i]) {
					...
					case "*":
						if (ts.repeat != null) {
							...
							if (isNaN((repeat = Number.parseInt(track.slice(ts.i + 1))))) {
		...
		edide.qs = (() =>
			(selector, el = document) =>
				el.querySelector(selector)).call((MODSELF = {}), edide, MODSELF);
				...
				if (
					...
					__usable_globalThis.requireScriptPromises.set(
						...
						(promise = edide.promise.new((resolve) =>
							edide.scriptAdd(scriptSrc, resolve),
						)),
					);
		...
		edide.mmPipe = function (edide, mmPipe) {
			...
			this.initInstrument = (instrument) => {
				...
				edide.onUnload(() =>
					instrument != null ? instrument.disconnect() : void 0,
				);
				return instrument;
			};
		...
		edide.mmInstruments = function (edide, mmInstruments) {
			...
			this.createNew = () => {
				...
				if (instrumentConf.module_name) {
					...
					var inst = new __usable_globalThis.Tone.Sampler(noteFiles, {
		...
		edide.cloneSibling = (() => (srcObj) => {
			var o = Object.create(Object.getPrototypeOf(srcObj));
