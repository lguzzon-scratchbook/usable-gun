...
export default function (__usable_environment) {
  ...
  k.down = (eve) => {
    const key = ((k.eve = m.eve = eve).which =
  ...
  m.list = (at, opt) => {
    ...
    __usable_globalThis.$.each(at, (i, k) => {
      i != 'back' && k && k.combo && k.name && l.push(k)
    })
    if (!l.length) {
      return
    }
    ...
    const $ul = __usable_globalThis.$('#meta .meta-menu ul')
    ...
    __usable_globalThis.$.each(l, (i, k) => {
      const $li = __usable_globalThis.$('<li>').text(k.name).data(k)
      $ul.append($li)
  ...
  m.ask = (help, cb, opt) => {
    const $ul = __usable_globalThis.$('#meta .meta-menu ul').empty()
    const $put = __usable_globalThis
      .$('<input>')
      .attr('id', 'meta-ask')
      .attr('placeholder', help)
    const $form = __usable_globalThis
      .$('<form>')
      .append($put)
      .on('submit', (eve) => {
        eve.preventDefault()
  ...
  __usable_globalThis.meta.edit = (e) => {
    ...
    let at = __usable_globalThis.meta.edit
    const l = e.combo.length
