  ...
  (() => {
    ...
    */
    ...
    function Store (opt) {
      ...
      store.start = () => {
        const o = __usable_globalThis.indexedDB.open(opt.file, 1)
    ...
    if (typeof __usable_window !== 'undefined') {
      (Store.window = __usable_window).RindexedDB = Store
    ...
    try {
      const Gun = Store.window.Gun || gunPlugin(__usable_environment)
