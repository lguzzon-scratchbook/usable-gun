...
export default function (__usable_environment) {
  ...
  function start (root) {
    ...
    if (
      typeof __usable_process !== 'undefined' &&
			`${(__usable_process.env || '').AXE}` === 'false'
    ) {
      return
    }
    ...
    GET.turn = (msg, route, turn) => {
      ...
      if (!tmp || !tag) {
        return
      } // message timed out, GUN may require us to relay, tho AXE does not like that. Rethink?
      ...
      next = __usable_globalThis
        .objectMaps(route || opt.peers)
        .slice((turn = turn || 0))
      if (!next.length) {
        if (!route) {
          return
        } // asked all peers, stop asking!
        GET.turn(msg, undefined, 0) // asked all subs, now now ask any peers. (not always the best idea, but stays )
        return
      }
      __usable_globalThis.settimeoutEach(
        next,
        (id) => {
          const peer = opt.peers[id]
          turn++
          if (!peer || !peer.wire) {
    ...
    function REF (msg) {
      ...
      if (typeof (soul = msg.get['#']) === 'string') {
        ref = root.$.get(soul)
      }
      if (typeof (tmp = msg.get['.']) === 'string') {
        has = tmp
      } else {
        has = ''
      }
      const via = (msg._ || '').via
      const sub = via.sub || (via.sub = new __usable_globalThis.objectMap());
      (
        sub.get(soul) ||
				(sub.set(soul, (tmp = new __usable_globalThis.objectMap())) && tmp)
      ).set(has, 1) // {soul: {'':1, has: 1}} // TEMPORARILY REVERT AXE TOWER TYING TO SUBSCRIBING TO EVERYTHING. UNDO THIS!
      via.id &&
				ref._ &&
				(
				  ref._.route || (ref._.route = new __usable_globalThis.objectMap())
				).set(via.id, via) // SAME AS ^
    ...
    root.on('in', function (msg) {
      ...
      if ((tmp = msg['@']) && (tmp = dup.s[tmp])) {
        ...
        if (tmp.it && tmp.it.get && msg.put) {
          ...
          const get = tmp.it.get || ''

          const ref = REF(tmp.it)._
          let via = (tmp.it._ || '').via || ''
          let sub
          if (via && ref) {
            ...
            via.id &&
							(
							  ref.route || (ref.route = new __usable_globalThis.objectMap())
							).set(via.id, via)
            sub = via.sub || (via.sub = new __usable_globalThis.objectMap())
            ref &&
							(
							  sub.get(LEX(get['#'])) ||
								(sub.set(
								  LEX(get['#']),
								  (sub = new __usable_globalThis.objectMap())
								) &&
									sub)
                ...
                if (has) {
                  (
                    sift ||
										(sub.set(
										  soul,
										  (sift = new __usable_globalThis.objectMap())
										) &&
											sift)
                  ).set(has, 1)
                } else if (!sift) {
                  sub.set(soul, (sift = new __usable_globalThis.objectMap()))
                  sift.set('', 1)
                }
              }
            }
        ...
        if ((tmp = tmp.back)) {
          ...
          __usable_globalThis.settimeoutEach(Object.keys(tmp), (id) => {
            to.next({
              '#': msg['#'],
              '@': id,
              ok: msg.ok
    ...
    root.on('create', function (root) {
      ...
      root.on('put', function (msg) {
        ...
        (ref.skip = {
          ...
          __usable_globalThis.settimeoutEach(
            __usable_globalThis.objectMaps(route),
            (pid) => {
              let peer
              var tmp
              const skip = ref.skip || ''
              ...
              const sub = (
                peer.sub || (peer.sub = new __usable_globalThis.objectMap())
              ).get(soul)
              if (!sub) {
                return
              }
              ...
              const put = peer.put || (peer.put = {})
              const node = root.graph[soul]
              var tmp
              if (node && undefined !== (tmp = node[has])) {
                state = state_is(node, has)
                val = tmp
              }
              put[soul] = state_ify(put[soul], has, state, val, soul)
              tmp = dup.track(
                (peer.next = peer.next || __usable_globalThis.stringRandom(9))
              );
    ...
    (() => {
      ...
      axe.up = {}
      const hi = mesh.hear['?'] // lower-level integration with DAM! This is abnormal but helps performance.
      mesh.hear['?'] = (msg, peer) => {
        let p // deduplicate unnecessary connections:
        hi(msg, peer)
        if (!peer.pid) {
          return
        }
      ...
      mesh.hear.opt = (msg, peer) => {
        ...
        if (Object.keys(axe.up).length >= 99) {
          return
      ...
      axe.stay = () => {
        ...
        axe.stay.to = setTimeout((tmp, urls) => {
          if (!(tmp = root.stats && root.stats.stay)) {
            return
          }
          urls = {}
          Object.keys(axe.up || '').forEach((p) => {
            p = (axe.up || '')[p]
            if (p.url) {
              urls[p.url] = {}
            }
      ...
      setTimeout((tmp) => {
        ...
        __usable_globalThis.settimeoutEach(tmp || [], (url) => {
          mesh.hear.opt({
            opt: {
              peers: url
            }
    ...
    (() => {
      ...
      */
      ...
      root.on('hi', function (peer) {
        ...
        const count = /* Object.keys(opt.peers).length || */ mesh.near // TODO: BUG! This is slow, use .near, but near is buggy right now, fix in DAM.
  ...
  (() => {
    ...
    __usable_globalThis.objectMaps = (o) => {
      if (from && o instanceof Map) {
        return from(o.keys())
      }
      if (o instanceof __usable_globalThis.objectMap) {
        o = o.s
      }
      return Object.keys(o)
    }
    if (from) {
      return (__usable_globalThis.objectMap = Map)
    }
    (__usable_globalThis.objectMap = function () {
      this.s = {}
    }).prototype = {
      set (k, v) {
        this.s[k] = v
