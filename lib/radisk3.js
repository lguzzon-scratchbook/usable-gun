...
export default async function (__usable_environment){
	...
	await (async () => {
		...
		let Book = __usable_globalThis.settimeoutBook;
		...
		var RAD =
			...
			(__usable_globalThis.settimeoutRAD = (opt) => {
				...
				let log = opt.log || nope;
				let has = (__usable_globalThis.settimeoutRADHas ||
					(__usable_globalThis.settimeoutRADHas = {}))[opt.file];
				if (has) {
					return has;
				}
				let r = function rad(word, is, reply){
						if (!b) {
							start(word, is, reply);
							return r;
						}
				...
				async function write(word, reply){
					...
					get(p, (err, disk) => {
						...
						put(p, `${  p}`, (err, ok) => {
							env.count--;
							p.saved = Number(new Date());
							log(
								"      ...wrote %d bytes in %dms",
								(`${  p}`).length,
								(p.saved = Number(new Date())) - S,
							);
							if (!p.saving.length) {
								p.saving = 0;
		...
		let nah = () => nope;
		env.require =
			(typeof __usable_require !== "undefined" && __usable_require) || nope;
		env.process = (typeof __usable_process !== "undefined" &&
			__usable_process) || {
			memoryUsage: nah,
		};
	...
	await (async () => {
		...
		let RAD = __usable_globalThis.settimeoutRAD;
