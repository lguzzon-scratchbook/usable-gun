		...
		Gun.chain.time = function (data, a, b) {
			...
			if (Gun.is(data)) {
				data.get((soul) => {
					...
					this.time(Gun.val.link.ify(soul), a, b);
				}, true);
				return this;
			}
			...
			this.get((soul) => {
				...
				p = id;
				if (!p) {
					id = p = (this.back("opt.uuid") || Gun.text.random)(9);
				}
				// could shrink this into a loop. Do later?
				t = [p].concat(t);
				var rid =
					__usable_globalThis.opt.key ||
					(this.back("opt.uuid") || Gun.text.random)(9);
			...
			root.get(t).get((msg, ev) => {
				...
				if (o.last && false !== off) {
					...
					var less = Gun.list.map(keys, (k) => {
						if (Number.parseFloat(k) < Number.parseFloat(next)) {
							return k;
						}
					});
					if (!less) {
						...
						tmp.push(less);
						(o.low = tmp.slice()).push(Number.POSITIVE_INFINITY);
						find(o, cb, root, tmp, true);
					}
				}
				if (off) {
