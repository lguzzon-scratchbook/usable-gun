...
export default function (__usable_environment) {
  ...
  (() => {
          ...
          if (replacer && typeof replacer === 'object') {
            ...
            for (itr = 0; itr < length; itr += 1) {
              if (typeof replacer[itr] === 'string') {
                ...
                if (val !== undefined) { result.push(normalize(key, 2) + (space ? ': ' : ':') + val) }
              }
            }
          } else {
            // Object case
            ...
            for (key in value) {
              ...
              objStack = objStack.filter((v) => v !== value[key])
            }
            objStack = objStack.filter((v) => v !== value)
          }
    ...
    const stringifyWrapper = (value, replacer, space, intensity, callback) => {
      let indent = ''
    ...
    /**
		 ...
		 * @returns {Function} YieldCPU.
		 */
    const parseWrapper = (text, reviver, intensity, cb) => {
      let counter = 0
      let keyN = 0
      let parseStr = text
      let at = 0
      ...
      const yieldCPU = () => {
        setTimeout(() => {
          ...
          if (gen && gen.done === true) {
            const isEmpty = (value) => {
              if (value.charAt(0) === '}' || value.charAt(0) === ']') { value = value.substring(1, value.length) }
              return typeof value === 'string' && !value.trim()
            }
            if (typeof yielding === 'undefined') { return cb(new ParseError('Unexpected Character'), null) } else if (yielding instanceof ParseError) return cb(yielding, null)
            else if (!isEmpty(parseStr)) { return cb(new ParseError('Unexpected Character'), null) } else {
              if (reviver !== null) {
    ...
    yson.parseAsync = (data, callback, reviver = null, intensity = 1) => {
      // Bring parity with the in-built parser, that takes both string and buffer
      if (__usable_globalThis.Buffer.isBuffer(data)) data = data.toString()
    ...
    /**
		 ...
		 * @param { number } intensity
		 * @param {Function} callback
		 * @returns {Function} StringifyWrapper.
		 */
    yson.stringifyAsync = (
      data,
      callback,
      ...
      if (typeof space === 'number' || typeof space === 'string') { space = validateSpace(space) }
  ...
  __usable_environment.exports.lib.ison = __usable_module.exports
