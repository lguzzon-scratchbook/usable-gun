...
export default function (__usable_environment) {
	...
	Gun.chain.promSet = async function (item, opt) {
		...
		return new Promise(async (res, rej) => {
			...
			if (!Gun.is(item)) {
				if (Gun.obj.is(item)) {
					item = await this.back(-1)
						.get(
							(soul = soul || Gun.node.soul(item) || this.back("opt.uuid")()),
						)
						.promPut(item);
					item = item.ref;
				}
				res(
					this.get(soul || Gun.state.lex() + Gun.text.random(7)).promPut(item),
				);
			}
			item.get((soul, o, msg) => {
				if (!soul) {
				...
				this.put(Gun.obj.put({}, soul, Gun.val.link.ify(soul)), cb, opt);
			}, true);
			res({
				ref: item,
				ack: {
