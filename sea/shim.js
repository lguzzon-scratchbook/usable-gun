...
export default function (__usable_environment) {
  ...
  api.parse = (t, r) => new Promise((res, rej) => {
    __usable_globalThis.jsonParseAsync(
      t,
      (err, raw) => {
        err ? rej(err) : res(raw)
  ...
  api.stringify = (v, r, s) => new Promise((res, rej) => {
    __usable_globalThis.jsonStringifyAsync(
      v,
      (err, raw) => {
        err ? rej(err) : res(raw)
  ...
  if (SEA.window) {
    ...
    api.random = (len) => api.Buffer.from(
  ...
  if (!api.crypto) {
    try {
      Object.assign(api, {
        ...
        random: (len) => api.Buffer.from(__usable_globalThis.crypto.randomBytes(len))
      })
