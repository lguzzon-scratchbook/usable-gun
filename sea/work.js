  ...
  SEA.work =
		...
		(async (data, pair, cb, opt) => {
		  try {
		    ...
		    let salt = (pair || {}).epub || pair // epub not recommended, salt should be random!
		    opt = opt || {}
		    ...
		    const work = await (shim.ossl || shim.subtle).deriveBits(
		      {
		        name: opt.name || 'PBKDF2',
		    ...
		    const r = shim.Buffer.from(work, 'binary').toString(
		      opt.encode || 'base64'
		    )
		    if (cb) {
		      try {
