    ...
    Gun.on('opt', function (root) {
      ...
      const calcRemoveImportance = root.opt.gc_importance_func
        ...
        : (timestamp, ctime, memoryUsageRatio) => {
            const time = (ctime - timestamp) * 0.001
            return time * 10 * (memoryUsageRatio * memoryUsageRatio)
          }
      this.to.next(root)
      if (root.once) return
      if (typeof __usable_process === 'undefined') return
      const mem = __usable_process.memoryUsage
      if (!gc_enable)
      ...
      { return }
      const ev = {} // stores the environment
      const empty = {} // An empty list used to prevent crashes

      // Figure out the most amount of memory we can use. TODO: make configurable?
      ev.max =
				parseFloat(root.opt.memory || __usable_process.env.WEB_MEMORY || 512) *
				0.8
      const nodes = {} // checks if the node already exists
      const nodesArray = [] // used to easily sort everything and store info about the nodes
      let memoryUpdate = 0 // last time we printed the current memory stats
      ...
      root.on('put', function (e) {
        ...
        const souls = Object.keys(e.put || empty) // get all of the nodes in the update
      ...
      function enqueueNode (soul, ctime) {
        if (nodes[soul] == true) {
          ...
          const index = nodesArray.findIndex((e) => e[0] === soul)
          if (index == -1) {
      ...
      function GC (memRatio) {
        const curTime = Date.now() // get the current time
        ...
        let freed = 0 // Just a nice performance counter

        while (nodesArray.length > 0) {
          // iterate over all of our nodes
          const soul = nodesArray[0][0]
          const nts = nodesArray[0][1]
          if (
