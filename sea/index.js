...
export default function (__usable_environment) {
	...
	var Gun = (SEA.window || "").GUN || gunPlugin(__usable_environment);
	...
	var valid = Gun.valid;
	var link_is = (d, l) => "string" == typeof (l = valid(d)) && l;
	...
	SEA.opt.pack = (d, cb, k, n, s) => {
		...
		__usable_globalThis.jsonParseAsync(f ? tmp : d, (err, meta) => {
			var sig = undefined !== (meta || "")[":"] && (meta || "")["~"]; // or just ~ check?
			if (!sig) {
				cb(d);
				return;
			}
	...
	SEA.opt.unpack = (d, k, n) => {
		...
		var s = Gun.state.is(n, k) || O.fall_state;
		if (
			d &&
			4 === d.length &&
			soul === d[0] &&
