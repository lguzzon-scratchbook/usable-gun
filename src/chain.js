...
export default function (__usable_environment) {
	...
	function input(msg, cat) {
		...
		var root = cat.root; // eve = event, at = data at, cat = chain at, sat = sub at (children chains).
		var gun = msg.$ || (msg.$ = cat.$);
		var at = (gun || "")._ || empty;
		var tmp = msg.put || "";
		var soul = tmp["#"];
			...
			if (!valid(tmp)) {
				...
				return __usable_globalThis.settimeoutEach(
					...
					(k) => {
						...
						if ("_" == k || undefined === (state = state_is(tmp, k))) {
							return;
						}
						cat.on("in", {
							$: gun,
	...
	function unlink(msg, cat) {
		...
		var change = undefined !== put["="] ? put["="] : put[":"];
		var root = cat.root;
		var link;
		var tmp;
		if (undefined === change) {
	...
	function ack(msg) {
		...
		if (
			!msg.put ||
			("string" == typeof get["."] && undefined === tmp[get["."]])
		) {
			if (undefined !== at.put) {
				return;
			}
